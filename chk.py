...
# OWNER BY FB: @scht.id TIKTOK: @masanto_id TELE: @imnoob59
init(autoreset=True)
...
# Configuration
DELAY_BETWEEN_CHECKS = 1  # Delay in seconds between checks
...
def worker(combo_queue, proxies, hits_file):
    """Worker thread function with fixed queue handling"""
    global checked, hits, fails, errors

    while running and not stop_event.is_set():
        combo = None  # Initialize combo variable
        try:
